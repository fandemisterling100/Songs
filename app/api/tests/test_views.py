...
class TestUserRegisterValidations(APITestCase):
    ...
    def test_existing_email(self):
        ...
        self.assertTrue("email" in response_json)
        self.assertTrue(
            response_json["email"][0] == f"The email '{email}' is not available."
        )
    ...
    def test_valid_password(self):
        ...
        response = self.client.post(url, data={"email": email, "password": password})
...
class TestUserAuthorization(APITestCase):
    ...
    def test_login_authorized(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Bearer " + response_json.get("access")
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestSongViews(APITestCase):
    def setUp(self):
        ...
        response = self.client.post(
            ...
            data={"email": self.email_user_1, "password": test_password},
        )
        response_json = response.json()
        ...
        response = self.client.post(
            reverse("api:token_obtain_pair"),
            data={"email": self.email_user_2, "password": test_password},
        )
        response_json = response.json()
    ...
    def test_create_songs(self):
        ...
        response_public = self.client.post(url, data=public_song, format="json")
        self.assertEqual(response_public.status_code, status.HTTP_201_CREATED)
        self.assertTrue("Song created" in response_public.data)
        response_private = self.client.post(url, data=private_song, format="json")
        self.assertEqual(response_private.status_code, status.HTTP_201_CREATED)
        self.assertTrue("Song created" in response_public.data)

        self.assertEqual(
            Song.objects.filter(private=False).count(), initial_public_songs + 1
        )
        self.assertEqual(
            Song.objects.filter(private=True).count(), initial_private_songs + 1
        )
    ...
    def test_read_songs(self):
        ...
        self.client.post(url_create, data=song, format="json")
        response = self.client.get(url_read)
    ...
    def test_valid_song_update(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.access_token_user_1)
        response = self.client.post(url_create, data=song, format="json")
        response_json = response.json()
        ...
        response = self.client.put(url_update, data=updated_song, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            response_json["Permission denied"],
            "This song is private, you can not update it.",
        )
    ...
    def test_valid_song_reading(self):
        ...
        self.assertEqual(
            response.json()["meta"]["total_results"], initial_user_1_songs + 1
        )
        ...
        self.assertEqual(
            response_json["Permission denied"],
            "This song is private, you can not retrieve it.",
        )
    ...
    def test_delete_song(self):
        ...
        self.assertEqual(
            response_json["Permission denied"],
            "This song is private, you can not delete it.",
        )
